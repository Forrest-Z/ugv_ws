...
void recognition(MxNetMtcnn& mtcnn, Mxnet_extract& extract, cv::Mat& img, const cv::Mat& data, const std::vector<std::string> labels, std::string& s)
{
	...
	for (int i = 0; i < face_info.size(); ++i)
	{
		...
		cout << "ID Grade :"<<result.min_distance << endl;
		if(result.min_distance < 1) {
			s = labels[result.index];
